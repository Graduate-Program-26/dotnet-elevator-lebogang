...
public class QueriesTest
{
    ...
    public async Task Handle_GetElevatorStatusById()
    {
        ...
        var handler = new GetElevatorStateQueryHandler(mockElevatorRepo.Object, mockFloorRepo.Object);
    ...
    [Fact]
    public async Task Handle_GetElevatorStatusById_InvalidID()
    {
        var floor = CreateFloor(3);
        var elevator = _elevatorFactory.CreateElevator();
        floor.AddWaitingPassenger(new Passenger(sourceFloor: 3, destinationFloor: 7));
        ...
        Action act = await handler.Handle(new GetElevatorStateQuery(Guid.NewGuid()), CancellationToken.None);

        act.Should().Throw<InvalidElevatorIdException>();
    }
}